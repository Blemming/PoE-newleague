<template>
	<main style="background-image:url('https://api.poe.house/uploads/15381705a2404b3e8464c54b4ceb38f5.jpg')" class="relative flex  bg-cover bg-fixed overflow-hidden">
		<section class="w-full flex flex-col items-center pt-16 bg-cover bg-fixed">
			<section class="relative w-full lg:w-5/6 border-poe mb-4">
				<div class="bg-poe-x text-yellow-poe-dark ">
					<nuxt />
				</div>
			</section>
		</section>
	</main>
</template>
